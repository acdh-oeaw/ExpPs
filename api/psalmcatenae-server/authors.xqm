...
module namespace authors = 'https://expps.acdh.oeaw.ac.at/psalmcatenae-server/authors';
...
declare
    %rest:GET
    %rest:path('/psalmcatenae-server/manuscripts/{$manuscript-name}/commentaryfragments/authors-distinct')
    %rest:produces('application/hal+json')
function authors:get-authors-distinct-of-commentaryfragments-of-manuscript($manuscript-name as xs:string){
  let $origin := try { request:header("Origin") } catch basex:http {'urn:local'}
  let $manuscript := switch ($manuscript-name)
    case 'vat-gr-754' return 'vat-gr-754-transcription.xml'
    case 'ambr-b-106-sup' return 'ambr-b-106-sup.xml'
    ...
    default return error(xs:QName('response-codes:_404'),'Wrong manuscript name in path')
    let $path := "/psalmcatenae-manuscripts/" || ``[`{$manuscript}`]``
    let $author-result-fragment := for $author in distinct-values(doc($path)//tei:seg[(@type = 'commentaryfragment') or (@type = 'hypothesis')]/@source) return "{ ""_links"" : { ""self"" : { ""href"" : ""/psalmcatenae-server/manuscripts/" || $manuscript-name || "/commentaryfragments/authors-distinct"" }}, ""author"" : """ || $author || """}"
    let $authors-as-json-result-fragment := string-join($author-result-fragment,',')
    let $authors-as-json := "{ ""_links"" : { ""self"" : { ""href"" : ""/psalmcatenae-server/manuscripts/" || $manuscript-name || "/commentaryfragments/authors-distinct"" }}, ""_embedded"" : { ""authors"" : [" || $authors-as-json-result-fragment || "]}}"
    return
  (<rest:response>
    <output:serialization-parameters>
        <output:media-type value="application/hal+json"/>
...
(:~
 : Returns a list of distinct authors of glosses in a given manuscript
 :)
declare
    %rest:GET
    %rest:path('/psalmcatenae-server/manuscripts/{$manuscript-name}/glosses/authors-distinct')
    %rest:produces('application/hal+json')
function authors:get-authors-distinct-of-glosses-of-manuscript($manuscript-name as xs:string){
  let $origin := try { request:header("Origin") } catch basex:http {'urn:local'}
  let $manuscript := switch ($manuscript-name)
    case 'vat-gr-754' return 'vat-gr-754-transcription.xml'
    case 'ambr-b-106-sup' return 'ambr-b-106-sup.xml'
    ...
    default return error(xs:QName('response-codes:_404'),'Wrong manuscript name in path')
    let $path := "/psalmcatenae-manuscripts/" || ``[`{$manuscript}`]``
    let $author-result-fragment := for $author in distinct-values(doc($path)//tei:seg[@type = 'glosse']/@source) return "{ ""_links"" : { ""self"" : { ""href"" : ""/psalmcatenae-server/manuscripts/" || $manuscript-name || "/glosses/authors-distinct"" }}, ""author"" : """ || $author || """}"
    let $authors-as-json-result-fragment := string-join($author-result-fragment,',')
    let $authors-as-json := "{ ""_links"" : { ""self"" : { ""href"" : ""/psalmcatenae-server/manuscripts/" || $manuscript-name || "/glosses/authors-distinct"" }}, ""_embedded"" : { ""authors"" : [" || $authors-as-json-result-fragment || "]}}"
    return
  (<rest:response>
    <output:serialization-parameters>
        <output:media-type value="application/hal+json"/>
