...
function cts:canonical-text-service($request as xs:string,$urn as xs:string){
  ...
  let $available-fragments := for $commentaryfragment in doc($path)/tei:TEI/tei:text/tei:body/tei:div[@type = 'psalm']/tei:div[@type = 'psalmverse']/tei:div[(@xml:lang = 'grc') and (@type = 'text')]/tei:div[(@type = 'commentary') and (not(exists(@rend)) or (@rend = 'parallel'))]/tei:p return $commentaryfragment/@n
  let $result-text := if ($language = 'grc') then for $commentaryfragment in doc($path)/tei:TEI/tei:text/tei:body/tei:div[@type = 'psalm']/tei:div[@type = 'psalmverse']/tei:div[(@xml:lang = 'grc') and (@type = 'text')]/tei:div[(@type = 'commentary') and (not(exists(@rend)) or (@rend = 'parallel'))]/tei:p where $commentaryfragment/@n = $passage return $commentaryfragment/text() else for  $commentaryfragment in doc($path)/tei:TEI/tei:text/tei:body/tei:div[@type = 'psalm']/tei:div[@type = 'psalmverse']/tei:div[(@xml:lang = 'grc') and (@type = 'text')]/tei:div[(@type = 'commentary') and (not(exists(@rend)) or (@rend = 'parallel'))]/tei:p where $commentaryfragment/@n = $passage return $commentaryfragment/parent::tei:div[@type = 'commentary']/parent::tei:div[@type = 'text']/parent::tei:div[@type = 'psalmverse']/tei:div[(@type = 'translation') and (@xml:lang = 'de')]/tei:p[substring-after(@corresp,'#') = $commentaryfragment/@xml:id]/text()
  let $result-xml-fragment := if (not(($namespace eq 'etf') and ($textgroup eq 'psath') and ($work eq 'expps') and (($language eq 'ger') or ($language eq 'grc'))))
     then 
       ...
       element reply {
         ...
         element source { "Heil, Uta; Panteghini, Sebastiano, (Ps.)-Athanasius, Expositiones in Psalmos. Fragment " || $passage || " - " || concat('urn:cts:etf:psath.expps.',$language,':',$passage) || " (https://expps.acdh-dev.oeaw.ac.at/expps/edition_ps_1-10.html)."}
       }
     }
  return
  (<rest:response>
