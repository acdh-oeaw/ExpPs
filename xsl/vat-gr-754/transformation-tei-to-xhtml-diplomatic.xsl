...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:teiHeader">
        ...
        <p class="date">
            <xsl:value-of select="'2023-04-12'"/>
        </p>
        ...
        <div class="manuscript-physical-description">
            ...
            <img class="layout-description-image">
                <xsl:attribute name="src">
                    <xsl:value-of select="tei:fileDesc/tei:sourceDesc/tei:msDesc/tei:physDesc/tei:objectDesc/tei:layoutDesc/tei:layout/tei:graphic/@url"/>
                </xsl:attribute>
            </img>
    ...
    <xsl:template match="tei:quote[@type = 'patristic']">
        ...
        <xsl:element name="span">
            <xsl:attribute name="id" select="concat('click-',$identifier)"/>    
            <i class="fas fa-info-circle"></i>
        </xsl:element>
        <xsl:element name="span">
            <xsl:attribute name="class" select="'vat-gr-754-diplomatic-hidden'"/>
        ...
        <xsl:element name="script">
            ...
            <xsl:text>$('#</xsl:text>
            <xsl:value-of select="concat('click-',$identifier)"/>
            <xsl:text>').click(function(){$('#</xsl:text>
            <xsl:value-of select="$identifier"/>
            <xsl:text>').toggleClass('vat-gr-754-diplomatic-visible');})</xsl:text>
        </xsl:element>
    </xsl:template>
